...
cell6 <- cell4 %>% 
  select(matches(cns_brain$ModelID))%>%
...
cell7 <- cell3 %>%
  ...
  select(gene_symbol, mean_score_all) %>%
  `rownames<-`( NULL )
...
cell8 <- cell3 %>% 
  ...
  mutate(gene_symbol = row.names(cell3)) %>%
  select(gene_symbol, mean_score_cns_brain) %>%
  `rownames<-`( NULL )
...
cells_n <- cell5 %>%
  ...
  select(gene_symbol, n_essential, percentage_essential) %>%
  inner_join(cell6 %>%
               mutate(gene_symbol = row.names(cell6)) %>%
               select(gene_symbol, n_essential_cns_brain, percentage_essential_cns_brain)) %>%
  rename(n_essential_all_1078 = n_essential,
...
cells_depmap <- cells_n %>%
  select(gene_symbol, percentage_essential_all,mean_score_all) %>%
